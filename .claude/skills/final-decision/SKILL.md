...
- **`horizon_basis: trading`을 쓰고 `horizon_days`를 거래일로 센다.** 달력일로 재면
  ...
  30일 판단이 같은 기간을 보지 않는다. 표준 사다리는 **거래일 20 / 40 / 60**이며
  (`scripts/lib/rules.mjs`의 `HORIZON_LADDER`가 이 값을 강제한다), 달력일에서 환산해
  쓰지 않는다 — 거래일 20일은 달력 20일이 아니라 28일 안팎이다. 더 긴 horizon은 retro
  seed로 만들 수 없고 forward 판단에서만 쓴다.
...
- **손절폭을 `horizon_days`에 맞춘다.** 위 규칙만으로는 조기 종료가 막히지 않는다.
  ...
  앞 세 행이 사다리 값이고, 90·120은 forward 판단에서만 나온다. 식의 분모 30은 달력
  한 달을 기준으로 잡은 상수이며 `minStopWidth`가 그대로 쓴다 — horizon을 거래일로
  세는 것과는 별개다.
